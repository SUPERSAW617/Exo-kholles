...
\end{exercise}
\begin{exercise}
1) Montrer que pour tout $n\in\N^*, \floor{\sqrt{n^4+2n^3+3n^2+1}}=n^2+n$.\\
2) Déterminer l'ensemble des entiers $n\in\N^*$ tels que $n^4+2n^3+3n^2+1$ est un carré parfait.
\end{exercise}
\begin{exercise}
1. Soit $A\subset\R$ non vide.\\
a. Justifier pour $x\in\R$ l'existence du réel $d(x,A)=\inf_{a\in A}|x-a|$.\\
b. Calculer $d(x,A)$ pour tout $x\in A$.\\
c. Montrer que pour tous $x,y\in\R$, on a $|d(x,A)-d(y,A)|\le|x-y|$.\\
2. On suppose ici que $A=\Q\cap[0,1[$. Déterminer $d(x,A)$ pour tout $x\in\R$ et tracer le graphe de la fonction $x\mapsto d(x,A)$.
\end{exercise}
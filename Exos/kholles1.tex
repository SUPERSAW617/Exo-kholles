...
\begin{exercise}
\begin{questions}
\item Soit $(x_n)_{\in\N}$ une suite de rééls telle que : $$\forall n \ge0, \sum_{=0}^nx_k^3=(\sum_{=0}^nx_k)^2$$ Montrer que pour tout $n\ge0$, il existe un entier naturel $m$ tel que : $$\sum_{=0}^nx_k=\frac{m(m+1)}{2}$$
\item On pose $S_{n,p}=\sume_{k=0}^nk^p$. Déterminer les entiers naturels non nuls $p$ tels que, pour tout $n\ge0$, $S_{n,p}$ soit le carré d'un entier naturel.
\end{questions}
\end{exercise}
